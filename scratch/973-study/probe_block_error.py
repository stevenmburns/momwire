    ...
    RECORDS.append(
        {
            ...
            "normA": nA,
            "abs_err": err,
            "rel_err": err / nA if nA > 0 else 0.0,
...
def main():
    for tol in (1e-6, 1e-8):
        ...
        print(
            f"  block rel err: max {rel.max():.3e}  median {np.median(rel):.3e}  "
            f"n>tol {int((rel > tol).sum())}"
        )
        worst = sorted(recs, key=lambda r: -r["rel_err"])[:6]
        print(
            f"  {'m':>4s} {'n':>4s} {'rank':>4s} {'rel_err':>10s} {'x tol':>8s} {'||A||':>10s}"
        )
        for r in worst:
            print(
                f"  {r['m']:4d} {r['n']:4d} {r['rank']:4d} {r['rel_err']:10.3e} "
