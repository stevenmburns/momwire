...
STAGE 2 RAN THOSE EXPERIMENTS AND NONE OF THEM FIRED
-----------------------------------------------------
All three of §5.4's candidates are now dead (see the study's stage-2
record). Candidate 1, the remainder's near-interface behaviour: bypassing
the interpolation grid entirely and evaluating the six lambda-integrals
directly at rtol 1e-11 moves the poor-soil residual 3.3274 -> 3.3305 ohm,
and raising the remainder's spatial quadrature order 3 -> 12 moves it
0.03 ohm. Candidate 2 died in stage 1 (the stub ladder, 0.011 ohm).
Candidate 3, a missing base-loss resistance: the discrepancy is LARGEST
over a lossless dielectric, which is what the `diel` row below records.

So the gap is a formulation difference at the contact node, the envelope
pins are permanent, and this lane grows one row rather than losing two.
...
# leading rows are the DECAY class and the two trailing soil rows the
# ENVELOPE class; `test_contact_nec5_lane.py` reads that split from here.
#
# `diel` is stage 2's addition and is not a soil. Its loss tangent is
# sigma/(omega eps0 eps_r) = 5.1e-3: a half-space that cannot dissipate. It
# is here because it is where the discrepancy is LARGEST (4.36 ohm at
# N = 41, against poor soil's 3.27), which is the measurement that killed
# study §5.4's third candidate — a missing base-LOSS resistance cannot be
# at its maximum over a ground with no loss in it. Keeping the row means
# that kill survives as a gate instead of as a paragraph.
GROUND_EPS = {
    "sea": (81.0, 5.0),
    "vgood": (20.0, 0.0303),
    "avg": (13.0, 0.005),
...
def _write_golden(rows) -> None:
    lines = [
        ...
        "(momwire#282 stages 1 and 2). Citation: NEC-5 (LLNL-CODE-746721),",
        "ground-contact ladder decks over four soils and one lossless",
        "dielectric, 2026-08-18 and 2026-08-19.",
        "",
