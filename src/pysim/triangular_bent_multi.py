...
Cross-product of `triangular_bent` (one bent polyline) and `triangular_yagi`
(multiple straight parallel wires): each wire is its own polyline of M >= 2
anchor points, edges within a wire may bend at kinks, and basis functions
are continuous through kinks within a wire but vanish at every wire's
endpoints (no continuity across wires).
...
class BentMultiPySim(AbstractPySim):
    ...
    n_per_edge_per_wire: list of (int or sequence). Per-wire segments per
        polyline edge. None for a wire means use `self.nsegs` for each of
        its edges; an int means use that count for each edge; a sequence
        gives a per-edge count. If `n_per_edge_per_wire` itself is None,
        every wire uses `self.nsegs` on every edge.
