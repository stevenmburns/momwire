...
class PySim:
    ...
    def jax_compute_impedance(self):
        ...
        minus at [1, 3, 5] and plus at [3, 5, 7]
        """
        exnm = jnp.linspace(p0-delta_p, p1+delta_p, 2*self.nsegs+3)
        ...
        delta_l = jnp.sqrt((vec_delta_l**2).sum(axis=1))
        delta_l_plus = jnp.sqrt((vec_delta_l_plus**2).sum(axis=1))
        delta_l_minus = jnp.sqrt((vec_delta_l_minus**2).sum(axis=1))
        ...
        def Integral(n, m, delta):
            ...
            diffs = n[jnp.newaxis, :, :] - m[:, jnp.newaxis, :]
            R = jnp.sqrt((diffs*diffs).sum(axis=2))

            assert n.shape[0] == delta.shape[0]

            # not always diagonal indices
            diag_indices = jnp.where(R == 0) # R < 0.0001)
            new_delta = delta[diag_indices[0]]

            assert new_delta.shape[0] == delta.shape[0] or new_delta.shape[0]+1 == delta.shape[0]

            #RR = R
            #RR[diag_indices] = 1
            RR = R.at[diag_indices].set(1)

            res = jnp.exp(-(0+1j)*self.k*R)/(4*jnp.pi*RR)
            diag = 1/(2*jnp.pi*new_delta) * jnp.log(new_delta/self.wire_radius) - (0+1j)*self.k/(4*jnp.pi) 
            #res[diag_indices] = diag
            res = res.at[diag_indices].set(diag)
            return res

        z = self.jomega * self.mu * (vec_delta_l[jnp.newaxis, :, :] * vec_delta_l[:, jnp.newaxis, :]).sum(axis=2)
