...
def Integral_Standalone(m, n, *, ntrap, wire_radius, k):
    ...
    res = np.zeros(
        shape=(m_centers.shape[0], n_endpoints.shape[0] - 1), dtype=np.complex128
    )
    if ntrap == 0:
        res += Aux(0.5)
    else:
...
class YagiPySim(AbstractPySim):
    ...
    def compute_impedance(self, *, ntrap=0):
        ...
        p0, p1 = (
            np.array((0, -y, 0), dtype=np.float64),
            np.array((0, y, 0), dtype=np.float64),
        )

        q0, q1 = (
            np.array((-x, -1.05 * y, 0), dtype=np.float64),
            np.array((-x, 1.05 * y, 0), dtype=np.float64),
        )
        ...
        def Integral(nodes_and_endpoints, ntrap):
            return Yagi_Integral_Standalone(
                nodes_and_endpoints, ntrap=ntrap, wire_radius=self.wire_radius, k=self.k
            )
