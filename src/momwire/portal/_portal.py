...
# owns — which closed the only eznec -> portal import (the ranked extraction
# backlog, momwire#429, is what marked that import as debt). Imported back
# here for this module's own internal use, under the ``as name`` idiom so the
# names stay module attributes: `from momwire.portal._portal import
...
from .._far_readout import (
    ...
    _cliff_image_moments as _cliff_image_moments,
    _cliff_medium_2 as _cliff_medium_2,
    _element_fields as _element_fields,
...
# imported above. ``_PRINTED_DUST_FLOOR2`` did not move — it is this
# printout's own departure, not a reading FFLD returns — so this module still
# owns it and the two floors are still "both spelt 1e-20 and NOT the same
# test" (issue #802), pinned apart by ``test_the_two_dust_floors_are_pinned_
# separately``.
#
...
# ``Ground`` moved to ``momwire._far_readout`` with the far-zone readout
# (momwire#719 U1) and is imported above; it stayed dialect-neutral even
# where it lived here (its docstring already said "reduced to what both the
# printout and momwire need"), so the move changed nothing about it.
...
# _image_moments / _image_coeffs / _cliff_medium_2 / _cliff_image_moments /
# _far_moments / _element_fields / _polarisation / _gain_db all moved to
# `momwire._far_readout` with the far-zone readout (momwire#719 U1) and are
# imported above where this module still needs them. `_phase_deg` (above)
# did NOT move: it is also used by this module's own row formatters
# (`fmt_pattern_row`, `fmt_near_field_row`), so `_far_readout._polarisation`
# carries its own copy rather than reaching back across the boundary for a
# two-line trig helper.
