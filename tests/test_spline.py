...
from pysim.spline import fit_test_case, solve_test_case, vector_test_case
...
def test_solve(tag):
    ...
    for NN in range(N, N + 1, 2):
        xs, rhs, exact_solution, estimated_rhs, estimated_solution = solve_test_case(
            tag=tag, N=NN, nsegs=nsegs
        )
...
def test_vector(tag):
    ...
    for NN in range(N, N + 1, 2):
        xs, rhs, exact_solution, estimated_rhs, estimated_solution = vector_test_case(
            tag=tag, N=NN, nsegs=nsegs
        )
