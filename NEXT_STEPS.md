...
- **`pysim.hmatrix.HMatrixPySim`** (in `src/pysim/hmatrix.py`) — distance-based hierarchical (H-matrix / ACA) accelerator, a subclass of `BSplinePySim` reusing its geometry/basis/kernels/KCL. Replaces the dense O(N²) assembly + LU solve with a cluster-tree block partition, Adaptive Cross Approximation low-rank far blocks, and a GMRES solve driven by a fast H-matvec + near-field sparse-LU preconditioner. numpy/scipy prototype (no C++ accelerator yet); free space + no-enrichment, with a dense fallback for ground/enrichment. Matches the dense bspline engine to ~1e-4 on rhombic / ~2e-5 on bowtiearray2x4; storage scales O(N log N) at constant far-block rank ~5 (49% → 6.4% of dense over n=250 → 4000). See [docs/hmatrix.md](docs/hmatrix.md). Next: C++ accelerators for the ACA fill + H-matvec to convert the proven operation-count win into wall-clock.
...
0. **Element-aware block-low-rank solver for arrays** (`ArrayBlockPySim`, planned) — the generic `HMatrixPySim` is geometry-blind and does *not* beat the dense path on the array driving examples at native size (`bowtiearray2x4` ~6× slower, 41% storage). Measured: an array's 8×8 element-block structure has weak (`‖Z_ab‖/‖Z_aa‖ ≈ 0.02`) and low-rank (~4) coupling, so an element-aligned decomposition (dense self-blocks + ACA coupling, identical-element + block-Toeplitz reuse, block-Jacobi GMRES) scales like `P·N²` not `(PN)²`, compresses to ~16%, and — key for animation — reuses the self-block factorization across phase/spacing-sweep frames. Full design + phased plan + validated measurements in [docs/array_block_solver_plan.md](docs/array_block_solver_plan.md).
...
- `scripts/compare_yagi_nec.py` — the NEC validation harness (single dipole and 2-element Yagi in free space). Requires PyNEC (install the release wheel — see the README's "Optional: PyNEC backend" section):
  ```
