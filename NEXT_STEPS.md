...
- **PR #37** — Fourth C++ accelerator: `assemble_Z_general` in `_accelerators.cpp` ports `_assemble_Z_general_batch` (the per-basis `(support_seg, support_L, support_R)` 2-wing assembly used for junction geometries). Same OpenMP `parallel for collapse(2)` over `(k, m)` as PR #13's fast path; inner n loop computes the full 2×2 (a, b) sum over wings, with `slope[m, a] = (R[m, a] - L[m, a]) / h[support_seg[m, a]]` precomputed once outside the parallel region. Junction directional bases (one wing inactive with L=R=0) pass through the same kernel — the inactive wing contributes zero by construction without a branch. Output matches the pure-Python reference to 3.3e-16 relative on a 5-band fan dipole. Kernel alone: **5286 → 45 ms at n_per_wire=21, n_k=41 (~117×)**, beating the ~33–66× estimate. End-to-end fan-dipole `/sweep` at n=21, n_k=41 goes **6067 → 971 ms (~6.25×)**, also beating the ~4× estimate. After PR #37, on the fan-dipole sweep workload the new bottleneck is `_build_J_blocks_batch` (~593 ms / 61%), with assembly down to ~5% and `np.linalg.solve` + KCL augmentation at ~33%. New regression tests: `test_assemble_Z_general_cpp_matches_python` (bit-exact C++ vs python on a K=3 junction) and `test_triangular_fandipole_swept_matches_per_freq` (batched-vs-per-freq for K=3, mirroring the existing K=2 test).
...
6. **C++ accelerator: remaining work after PR #37.** Phases 1, 2, 3, and 4 landed: cross-edge/cross-wire quadrature (PR #11), same-edge regularized quadrature (PR #12), Z matrix assembly fast path (PR #13), and Z matrix assembly general path for junction geometries (PR #37). Both triangular solver paths' Python overhead is now down to glue (geometry setup, the call sequence, np.linalg.solve, J build). Remaining wins, ranked by current Amdahl share:
   ...
   - ~~**6g. assemble_Z for junction geometries.**~~ — done in PR #37. `_assemble_Z_general_batch` collapses from 5286 ms → 45 ms at n_per_wire=21, n_k=41 (~117×); end-to-end fan-dipole /sweep 6067 → 971 ms (~6.25×). Bit-exact with the pure-Python reference to 3.3e-16 relative. After PR #37, **`_build_J_blocks_batch` is the new dominant cost** on the multi-band fan dipole sweep (~593 ms / 61%); see 6h.

   - **6h. J-build batching for junction multi-edge geometries.** *Now the dominant cost on /sweep for fan dipole.* `_build_J_blocks_batch` does a triple nested loop over wires × edges-per-wire × edges-per-wire and dispatches to either the same-edge analytic+regularized path or the off-edge quadrature path for each pair. For a 5-band fan dipole at n_per_wire=21 / n_k=41, this is **~593 ms / 61% of the sweep**. The kernels themselves (`seg_seg_quad_batch_3d`, `seg_seg_reg_quad_batch_1d`) are already in C++ from PR #11/#12 — the time is in Python-side dispatch overhead from the many small per-edge-pair calls (5 bands × 2 edges-per-arm × ~K edge pairs per wire pair). Two options: (a) hoist the per-edge loop into C++ by passing edge offsets and dispatching per pair internally; (b) coalesce many small same-wire off-edge pairs into one large batched quadrature call. Option (a) is more surgical; option (b) needs the same `a²` regularization, which is uniform, so it's easy to fuse. Estimated 3–5× on `_build_J_blocks_batch` for many-band geometries; smaller wins (1–2×) on the inverted V which only has 2 edges per wire. Next-priority on multi-band geometries.

   - **6d. Batched LU solve.** *Was the dominant cost on /sweep for inverted V before junction work landed.* `compute_impedance_swept` calls `np.linalg.solve(Z, v)` on a `(n_k, M, M)` matrix — already batched, **288 ms at N=80 (50%)** and **1239 ms at N=160 (60%)** for inverted V. On the 5-band fan dipole post-PR-#37 it's ~33% of the sweep (~325 ms). Per-thread LU-factorize-then-solve via LAPACK's `zgetrf`/`zgetrs` directly (parallel over k) should beat numpy's stacked call for our (n_k=41, M~160-320) shape: LAPACK's batched path may serialize internally, and OpenMP over the n_k axis gives near-linear scaling on this shape. Estimated 2–4× on the solve, hence ~1.5–2× end-to-end at N=160 / inverted V.
