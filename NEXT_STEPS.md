...
6. **C++ accelerator: remaining work after PR #12.** Phases 1 and 2 landed: cross-edge/cross-wire quadrature (PR #11) and same-edge regularized quadrature (PR #12). Combined, the C++ kernels are now ~16% of V `/sweep` time at N=80 (down from 70%+). Remaining wins, ranked by current Amdahl share:

   - **6e. C++-side matrix assembly.** *Now the dominant cost on /sweep.* The numpy einsum / fancy-indexing block in `compute_impedance_swept` that combines `(n_k, N, N)` J tensors plus tangent dot-products into the final `(n_k, n_basis, n_basis)` Z matrix is **488 ms at N=80 and 2380 ms at N=160** in the post-PR-#12 profile (60% of total at N=160). The structure (left/right segment indexing, divergence stencil, tangent dot products) is well-defined; a dedicated C++ kernel that takes the four J tensors plus `tangents`, `h_per_seg`, `omega_array`, `eps`, `mu` and emits Z directly should win big — probably 5–10× on this block, hence ~2× on full /sweep at N=160. Worth doing before 6b/6d because it dominates at the N we care about.

   - **6d. Batched LU solve.** `compute_impedance_swept` calls `np.linalg.solve(Z, v)` on a `(n_k, M, M)` matrix — already batched, ~609 ms at N=80 and ~878 ms at N=160 (15–22% of /sweep). Per-thread LU-factorize-then-solve via LAPACK's `zgetrf`/`zgetrs` directly may beat numpy's stacked call for our (n_k=41, M~160-320) shape. Lower-impact than 6e but contiguous in scope.

   - **6b. Vectorized `cexp(-jkR)` via SLEEF or Intel SVML.** The C++ kernels (both 1D and 3D) call `std::cos`/`std::sin` from libm one element at a time. Vectorized AVX2/AVX-512 sincos (4 or 8 elements per instruction) is realistically another 2–3× on each kernel, but the kernels are now ~16% of total — so end-to-end this would be ~5–10% on /sweep. Adds a build dependency. Defer until 6e/6d are done.
